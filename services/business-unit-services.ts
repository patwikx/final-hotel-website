...
// Define a type for the data needed to create a new Business Unit.
// We omit fields that are auto-generated by the database.
export type CreateBusinessUnitData = Omit<BusinessUnit, 'id' | 'createdAt' | 'updatedAt'>;

// Define a type for the data needed to update a Business Unit.
// All fields are optional, so you only send the data that has changed.
export type UpdateBusinessUnitData = Partial<CreateBusinessUnitData>;
 ...
 * @param {CreateBusinessUnitData} data - The data for the new business unit.
 * @returns {Promise<BusinessUnit>} A promise that resolves to the newly created business unit.
 * @throws {Error} Throws an error if the API request fails.
 */
export const createBusinessUnit = async (data: CreateBusinessUnitData): Promise<BusinessUnit> => {
  try {
